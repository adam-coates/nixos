...
//   property var vpnList: []         // [{name, active}]  — WireGuard/NM VPNs
//
//   // OpenConnect VPN state
//   property var ocVpns: [
//     { name: "Uni Graz VPN", gateway: "univpn.uni-graz.at", group: "Bedienstete" }
//   ]
//   property bool ocConnected: false
//   property bool ocConnecting: false
//
...
//     ocStatusProc._output = ""
//     ocStatusProc.running = false
//     ocStatusProc.running = true
//   }
//
//   Timer {
//     interval: 8000
//     running: netPanel.showing && !netPanel.ocConnecting
//     repeat: true
//     onTriggered: netPanel.refreshAll()
//   }
//
//   // Faster poll while waiting for SAML auth to complete
//   Timer {
//     interval: 2000
//     running: netPanel.ocConnecting
//     repeat: true
//     onTriggered: {
//       ocStatusProc._output = ""
//       ocStatusProc.running = false
...
//   }
//
//   // VPN toggle (WireGuard/NM VPNs)
//   Process { id: vpnToggleProc; onExited: netPanel.refreshAll() }
//
//   // OpenConnect VPN status
//   Process {
//     id: ocStatusProc
//     command: ["qs-vpn", "oc-status"]
//     property string _output: ""
//     stdout: SplitParser { onRead: line => ocStatusProc._output = line }
//     onExited: {
//       var wasConnecting = netPanel.ocConnecting
//       netPanel.ocConnected = ocStatusProc._output.trim() === "CONNECTED"
//       // Clear "connecting" state once VPN is actually up
//       if (netPanel.ocConnected && wasConnecting) netPanel.ocConnecting = false
//       ocStatusProc._output = ""
//     }
//   }
//
//   // OpenConnect VPN connect (opens browser for SAML auth)
//   // This process blocks while connected — it exits when VPN disconnects
//   Process {
//     id: ocConnectProc
//     onExited: (code) => {
//       netPanel.ocConnecting = false
//       netPanel.ocConnected = false
//       netPanel.refreshAll()
//     }
//   }
//
//   // OpenConnect VPN disconnect
//   Process {
//     id: ocDisconnectProc
//     command: ["qs-vpn", "oc-disconnect"]
//     onExited: netPanel.refreshAll()
//   }
//
//   function connectOpenConnect(gateway, group) {
//     ocConnecting = true
//     ocConnectProc._output = ""
//     ocConnectProc.command = ["qs-vpn", "oc-connect", gateway, group]
//     ocConnectProc.running = false
//     ocConnectProc.running = true
//   }
//
//   function disconnectOpenConnect() {
//     ocDisconnectProc.running = false
//     ocDisconnectProc.running = true
//   }
//
...
//         }
//
//         // OpenConnect VPN entries (SAML/SSO — managed by qs-vpn script)
//         Repeater {
//           model: netPanel.ocVpns
//
//           Rectangle {
//             required property var modelData
//             Layout.fillWidth: true
//             height: 36; radius: 4
//             color: netPanel.ocConnected ? Theme.accentAlpha(0.1)
//                  : ocItemHover.containsMouse ? Theme.accentAlpha(0.05)
//                  : "transparent"
//
//             RowLayout {
//               anchors { fill: parent; leftMargin: 8; rightMargin: 8 }
...
//                 font.family: Theme.fontFamily
//                 font.pixelSize: 14
//                 color: netPanel.ocConnected ? Theme.accent : Theme.gray
//               }
//
//               ColumnLayout {
//                 Layout.fillWidth: true
...
//                   font.family: Theme.fontFamily
//                   font.pixelSize: 11
//                   color: netPanel.ocConnected ? Theme.accent : Theme.fg
//                   Layout.fillWidth: true
//                   elide: Text.ElideRight
//                 }
//
//                 Text {
//                   text: netPanel.ocConnecting ? "Authenticating..." :
//                         netPanel.ocConnected ? "Connected" : "OpenConnect"
//                   font.family: Theme.fontFamily
//                   font.pixelSize: 9
//                   color: netPanel.ocConnected ? Theme.accent : Theme.gray
//                 }
...
//               Rectangle {
//                 width: ocBtnLabel.implicitWidth + 16
//                 height: 22; radius: 4
//                 color: netPanel.ocConnecting ? Theme.bg2
//                      : netPanel.ocConnected ? Theme.red + "33"
//                      : Theme.accentAlpha(0.2)
//
//                 Text {
//                   id: ocBtnLabel
//                   anchors.centerIn: parent
//                   text: netPanel.ocConnecting ? "..." :
//                         netPanel.ocConnected ? "Disconnect" : "Connect"
//                   font.family: Theme.fontFamily
//                   font.pixelSize: 10
//                   color: netPanel.ocConnecting ? Theme.gray
//                        : netPanel.ocConnected ? Theme.red
//                        : Theme.accent
//                 }
//
//                 MouseArea {
//                   anchors.fill: parent
//                   cursorShape: netPanel.ocConnecting ? Qt.WaitCursor : Qt.PointingHandCursor
//                   enabled: !netPanel.ocConnecting
//                   onClicked: {
//                     if (netPanel.ocConnected)
//                       netPanel.disconnectOpenConnect()
//                     else
//                       netPanel.connectOpenConnect(modelData.gateway, modelData.group)
//                   }
//                 }
//               }
//             }
//
//             HoverHandler { id: ocItemHover }
//           }
//         }
//
//         // WireGuard / NetworkManager VPN entries
//         Repeater {
...
//             Layout.fillWidth: true
//             height: 32; radius: 4
//             color: modelData.active ? Theme.accentAlpha(0.1) : vpnItemHover.containsMouse ? Theme.accentAlpha(0.05) : "transparent"
//
//             RowLayout {
//               anchors { fill: parent; leftMargin: 8; rightMargin: 8 }
//               spacing: 6
...
PanelWindow {
  ...
  property bool ocConnected: false
  property bool ocConnecting: false
  property int ocPolls: 0
  property string ocError: ""
  ...
  Process {
    ...
    onExited: (code) => {
      if (code !== 0) netPanel.connectError = "Wrong password or connection failed"
        ...
        netPanel.connectingSsid = ""
        netPanel.connectError = ""
      }
      netPanel.refreshAll()
    }
  }
  ...
  Process {
    ...
    stdout: SplitParser { onRead: line => ocStatusProc._output = line }
    onExited: {
      var status = ocStatusProc._output.trim()
      netPanel.ocConnected = status === "CONNECTED"

      if (netPanel.ocConnecting) {
        netPanel.ocPolls += 1

        if (netPanel.ocConnected) {
          netPanel.ocConnecting = false
          netPanel.ocError = ""
        } else if (netPanel.ocPolls >= 15) {
          netPanel.ocConnecting = false
          netPanel.ocError = "VPN login did not complete"
        }
      }
  ...
  // OpenConnect VPN connect (opens browser for SAML auth)
  // Started detached so the UI process is not tied to the VPN session lifetime
  Process {
    id: ocConnectProc
    property string _log: ""
    stdout: SplitParser { onRead: line => ocConnectProc._log += line + "\n" }
    stderr: SplitParser { onRead: line => ocConnectProc._log += line + "\n" }
    onExited: (code) => {
      if (netPanel.ocConnecting && !netPanel.ocConnected && code !== 0) {
        netPanel.ocConnecting = false
        netPanel.ocError = ocConnectProc._log.trim() !== "" ? ocConnectProc._log.trim() : "VPN connection failed"
      }
      netPanel.refreshAll()
      ocConnectProc._log = ""
    }
        ...
        Repeater {
          ...
          Rectangle {
            ...
            RowLayout {
              ...
              Text {
                text: modelData.security ? "\u{f0924}" : "\u{f05a9}"
                font.family: Theme.fontFamily
                font.pixelSize: 14
                color: Theme.fg
              }
        ...
        Repeater {
          ...
          Rectangle {
            ...
            ColumnLayout {
              ...
              Text {
                visible: netPanel.ocError !== ""
                text: netPanel.ocError
                font.family: Theme.fontFamily
                font.pixelSize: 9
                color: Theme.red
                Layout.fillWidth: true
